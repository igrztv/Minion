...
void Robot::getResponse()
{
	...
	while(Size > 0)
	{
		...
		if(moveStage.size() > 0)//åñëè åñòü òåêóùèå çàäàíèÿ
		{
...
{
	if(moveStage.size() > 0)//åñëè åñòü çàäàíèÿ
	{
		if(moveStage[0].done1 & moveStage[0].done0)//åñëè çàäàíèå çàâåðøåíî (êîëåñà äîåõàëè)
		{
			moveStage.erase(moveStage.begin());//óäàëÿåì èç î÷åðåäè
			previous_task_done = true;//ãîâîðèì, ÷òî ìîæíî äàâàòü ñëåäóþùåå çàäàíèå			
		}
		if(previous_task_done)//åñëè ïðåäûäóùåå çàäàíèå âûïîëíåíî
		{
			if(moveStage.size() > 0)////åñëè åñòü åùå çàäàíèÿ
			{
				path(moveStage[0].forthcoming_distance0);//çàäàåì ïóòü
				move(moveStage[0].speed0,moveStage[0].speed1);//çàäàåì ñêîðîñòü
				previous_task_done = false;//âûñòàâëÿåì ôëàã îæèäàíèÿ çàâåðøåíèÿ çàäàíèÿ
			}
		}
	}
}
...
{
	for(int i = 0; i < lidar.object.size(); i++)//äëÿ êàæäîãî íàéäåííîãî ó÷àñòêà (ïðåäïîëàãàåìîãî îáúåêòà)
	{
